...
func (m *module) getEarliestMetricTime(ctx context.Context, metricNames []string) (uint64, error) {
	...
	if err := m.telemetryStore.DB().QueryRow(ctx, query, args...).Scan(&minFirstReported); err != nil {
		return 0, err
	}
...
func (m *module) getMetricsExistence(ctx context.Context, metricNames []string) (map[string]bool, error) {
	...
	rows, err := m.telemetryStore.DB().Query(ctx, query, args...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (m *module) getMetadata(
	...
	rows, err := m.telemetryStore.DB().Query(ctx, outerQuery, innerArgs...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
