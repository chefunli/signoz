...
// DB returns a generic Conn wrapper around the mock Clickhouse connection.
func (p *Provider) DB() telemetrystore.Conn {
	return clickhousetelemetrystore.NewConnWrapper(p.clickhouseDB.(clickhouse.Conn))
}
...
func (p *Provider) Estimate(ctx context.Context, stmt string, args ...any) ([]telemetrystoretypes.EstimateEntry, error) {
	return clickhousetelemetrystore.RunExplainEstimate(ctx, p.DB(), stmt, args...)
}

// Plan runs EXPLAIN PLAN against the mock connection.
func (p *Provider) Plan(ctx context.Context, stmt string, args ...any) error {
	return clickhousetelemetrystore.RunExplainPlan(ctx, p.DB(), stmt, args...)
}
...
func (p *Provider) Indexes(ctx context.Context, stmt string, args ...any) (telemetrystoretypes.Granules, bool, error) {
	return clickhousetelemetrystore.RunExplainIndexes(ctx, p.DB(), stmt, args...)
}
